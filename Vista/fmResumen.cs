    ...
    public partial class fmResumen : Form
    {
        ...
        private void mtEstablecerPredeterminado()
        {
            try
            {
                ...
                var vector = objControl.mtConsultarDatosBasicos();
                
                    for (int j = 0; j <= 5; j++)
                    {
                    //cargar datos de los coches
                    //  vectorObj[j, 0].Text = vector[4];
                    vectorObj[j, 0].BackgroundImage = Image.FromFile(vector[2]); 
                         vectorObj[j, 1].Text = vector[0];
                         vectorObj[j,2].Text= vector[4];
                         vectorObj[j,3].Text = vector[5] + " KM";
                         vectorObj[j,4].Text = vector[1];
                         vectorObj[j, 5].Text = vector[3];
                }
